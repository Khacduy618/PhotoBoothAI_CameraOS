...
export const FinalPreviewScreen: React.FC<FinalPreviewScreenProps> = ({
  ...
  const [outputs, setOutputs] = useState<{ master: string; share: string; print: string } | null>(
    session.outputs?.master && session.outputs.share && session.outputs.print
      ? { master: session.outputs.master, share: session.outputs.share, print: session.outputs.print }
      : null
  );
