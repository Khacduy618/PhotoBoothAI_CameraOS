import type { DetectedSlot, ImportedFrameShotCount, FrameImportStatus } from "./frame-import.types";

const SUPPORTED_SHOT_COUNTS = new Set<number>([1, 2, 4, 6, 8]);

export function inferShotCount(count: number): ImportedFrameShotCount | null {
    return SUPPORTED_SHOT_COUNTS.has(count)
        ? (count as ImportedFrameShotCount)
        : null;
}

export function calculateConfidence(slots: readonly DetectedSlot[]): number {
    if (slots.length === 0) {
        return 0;
    }
...
export function classifyConfidence(confidence: number): FrameImportStatus {
    if (confidence >= 0.9) {
...
function estimateAlignmentScore(slots: readonly DetectedSlot[]): number {
    if (slots.length <= 2) {
        return 1;
    }
...
function estimateSpacingScore(slots: readonly DetectedSlot[]): number {
    if (slots.length <= 2) {
        return 1;
    }
    return 0.85;
