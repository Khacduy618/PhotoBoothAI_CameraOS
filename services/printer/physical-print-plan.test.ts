...
describe('resolvePhysicalPrintPlan', () => {
  it('A. PREMIUM_POSTCARD -> 1181x1748, 1 requested copy -> 1 sheet', () => {
    const plan = resolvePhysicalPrintPlan({
      product: 'PREMIUM_POSTCARD',
      requestedQuantity: 1,
    });
    expect(plan.productType).toBe('PREMIUM_POSTCARD');
    expect(plan.targetWidthPx).toBe(1181);
    expect(plan.targetHeightPx).toBe(1748);
  ...
  it('B. SHEET_4 -> 1181x1748, 3 requested copies -> 3 sheets', () => {
    const plan = resolvePhysicalPrintPlan({
      product: 'SHEET_4',
      requestedQuantity: 3,
    });
  ...
  it('D. STRIP_2: requested strips=2 -> 1181x1748 image, 2 duplicates per sheet, physical copies=1', () => {
    const plan = resolvePhysicalPrintPlan({
      product: 'STRIP_2',
      requestedQuantity: 2,
    });
  ...
  it('Handles landscape sheet orientation: 1748x1181', () => {
    const plan = resolvePhysicalPrintPlan({
      product: 'PREMIUM_POSTCARD',
      requestedQuantity: 2,
      isLandscape: true,
    });
    expect(plan.targetWidthPx).toBe(1748);
    expect(plan.targetHeightPx).toBe(1181);
