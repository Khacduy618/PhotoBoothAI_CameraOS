...
describe('buildPrintMaster & CP1000 Physical Raster', () => {
  ...
  it('D. STRIP_2 -> 1181x1748 with left (590px) and right (591px) duplication', async () => {
    ...
    // Call 1: Left strip drawn inside safe margins (Left: 35px, Top: 47px)
    const call1 = drawImage.mock.calls[0];
    expect(call1[0]).toBe(stripCanvas);
    expect(call1[5]).toBe(35); // destX (insetLeft = 3mm = 35px)
    expect(call1[6]).toBe(47); // destY (insetTop = 4mm = 47px)
    expect(call1[7]).toBe(556); // destWidth
    expect(call1[8]).toBe(1666); // destHeight (1748 - 47 - 35)

    // Call 2: Right strip drawn inside safe margins (Right: 35px)
    const call2 = drawImage.mock.calls[1];
    expect(call2[0]).toBe(stripCanvas);
    expect(call2[5]).toBe(591); // destX (insetLeft + leftStripW = 35 + 556)
    expect(call2[6]).toBe(47); // destY (insetTop = 4mm = 47px)
    expect(call2[7]).toBe(555); // destWidth
    expect(call2[8]).toBe(1666); // destHeight

    // Total content width = 556 + 555 = 1111 px (1181 - 2*35)
    expect(call1[7] + call2[7]).toBe(1111);
  });
  ...
  it('H. Landscape PREMIUM_POSTCARD produces 1748x1181 physical master', async () => {
    ...
    expect(call[5]).toBe(35);
    expect(call[6]).toBe(47);
    expect(call[7]).toBe(1678);
    expect(call[8]).toBe(1099);
  });
